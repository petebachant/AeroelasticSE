...
# See also  mlife
    ...
    dnotflat = np.array(data[:-1][(data[1:] - data[0:-1] !=0)])
...
def count_rainflow(col):
    ...
    ind = cycleRanges != 0
    cycleRanges = cycleRanges[ind]
    cycleCounts = cycleCounts[ind]
    return cycleRanges, cycleCounts
...
def rain_one(col, slope):
    ...
    cycleRanges, cycleCounts = count_rainflow(col)
